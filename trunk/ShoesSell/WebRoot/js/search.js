...
function fillTable(people) {
  ...
  for (i = 0; i < people.length; i++) {
    if (pattern.test(people[i].username)) {
  ...
  if (filtered.length == 0) {
    ...
    dwr.util.addRows("peoplebody", filtered, [
      function(person) { return person.id; },
      function(person) { return person.username.replace(pattern, "<span class='highlight'>$1</span>"); },
      function(person) { return "$" + person.password; }
...
function filterChanged() {
  ...
  if (filter.length == 0) {
    ...
    if (filter.charAt(0) == lastFilter.charAt(0)) {
      ...
      search.getMatchingFromLargeCrowd(filter.charAt(0), fillTable);
    }
  }
  lastFilter = filter;
}
